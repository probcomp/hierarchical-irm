...
BOOST_AUTO_TEST_CASE(test_dirty_can_equal_clean) {
  Sometimes<int> sc(new Categorical(5), true);

  double orig_lp = sbf.logp_score();
  BOOST_TEST(sbf.N == 0);
  sbf.incorporate(std::make_pair<int, int>(7, 8));
  BOOST_TEST(sbf.logp_score() < 0.0);
  BOOST_TEST(sbf.N == 1);
  sbf.unincorporate(std::make_pair<int, int>(7, 8));
  BOOST_TEST(sbf.logp_score() == orig_lp);
  BOOST_TEST(sbf.N == 0);

  sbf.incorporate(std::make_pair<int, int>(8, 7));
  sbf.incorporate(std::make_pair<int, int>(8, 7));
  BOOST_TEST(sbf.logp_score() < 0.0);
  BOOST_TEST(sbf.N == 2);

  BOOST_TEST(sbf.logp(std::make_pair<int, int>(7, 8)) < 0.0);
}
