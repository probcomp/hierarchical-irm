...
void topn_alignments(int n, const std::string& s1, const std::string& s2,
  ...
  heap.push_back(std::make_pair(0.0, empty_alignment));
  int num_found = 0;
  ...
  while ((num_found < n) && heap.size() > 0) {
    ...
    heap.push_back(std::make_pair(-del_cost, del));
    ...
    heap.push_back(std::make_pair(-ins_cost, ins));
    std::push_heap(heap.begin(), heap.end());
    ...
    heap.push_back(std::make_pair(-next_cost, next));
    std::push_heap(heap.begin(), heap.end());
  }
}
