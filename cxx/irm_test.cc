...
BOOST_AUTO_TEST_CASE(test_irm_one_observation) {
  std::map<std::string, T_relation> schema1{
      {"R1", T_clean_relation{{"D1"}, DistributionSpec("bernoulli")}},
      {"R2", T_clean_relation{{"D1"}, DistributionSpec("normal")}},
      {"R3", T_clean_relation{{"D1", "D1"}, DistributionSpec("normal")}}};
  IRM irm(schema1);
  ...
  double logp_y = irm.logp({{"R2", {1,}, obs1}}, &prng);
  BOOST_TEST(logp_y < 0.0);

  double logp_z = irm.logp({{"R2", {1,}, obs2}}, &prng);
